...
trap 'sudo rm -rf "${TMP_FILES[@]}"' EXIT
...
EOF
    sudo gpg --batch --import /tmp/public-key
    sudo curl -fsSL "https://awscli.amazonaws.com/awscli-exe-linux-${ARCH}.zip.sig" -o /tmp/awscliv2.sig
    sudo gpg --verify /tmp/awscliv2.sig /tmp/awscliv2.zip
