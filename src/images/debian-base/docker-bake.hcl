...
variable "AWS_ECR_PUBLIC_IMAGE_URI" {
  default = "public.ecr.aws/dev1-sg/devcontainer/debian-base:latest"
}

target "metadata" {
  labels = {
    "org.opencontainers.image.title"       = "${AWS_ECR_PUBLIC_IMAGE_NAME}"
    "org.opencontainers.image.description" = "${AWS_ECR_PUBLIC_IMAGE_NAME} ${AWS_ECR_PUBLIC_REPOSITORY_GROUP} image for internal use"
